...
from pv_workshop.schema_packages.pg_pv_package import (
    pg_pv_EQEmeasurement,
    pg_pv_JVmeasurement,
    pg_pv_Measurement,
    pg_pv_SimpleMPPTracking,
)
...
class RawFilepg_pv(EntryData):
    processed_archive = Quantity(
        type=Activity,
    )


class pg_pvParser(MatchingParser):
    def parse(self, mainfile: str, archive: EntryArchive, logger):
        ...
        entry = pg_pv_Measurement()

        if measurment_type == 'jv':
            entry = pg_pv_JVmeasurement()
        if measurment_type == 'eqe':
            entry = pg_pv_EQEmeasurement()
        if measurment_type == 'mppt':
            entry = pg_pv_SimpleMPPTracking()
        ...
        archive.data = RawFilepg_pv(processed_archive=get_reference(archive.metadata.upload_id, eid))
